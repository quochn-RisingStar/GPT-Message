...
class OpenAIService: @unchecked Sendable {
    ...
    private func makeJSONBody(with input: String, mode: Mode? = nil, stream: Bool = true) throws -> Data {
        ...
        switch mode {
        case .chat:
            let request = Chat(model: configuration.model.rawValue,
                               temperature: configuration.temperature,
                               messages: trimConversation(with: input),
                               stream: stream)
            return try JSONEncoder().encode(request)
        case .edits:
            let instruct = Instruction(instruction: input,
                                       model: configuration.model.rawValue,
                                       input: "")
            return try JSONEncoder().encode(instruct)
        case .completions:
            let command = Command(prompt: input,
                                  model: configuration.model.rawValue, maxTokens: 2048 - input.count,
                                  temperature: configuration.temperature, stream: stream)
...
extension String: CustomNSError {
    ...
    var isImageGenerationPrompt: Bool {
        lowercased().hasPrefix("draw") || lowercased().hasPrefix("vẽ") || lowercased().hasPrefix("image")
    }
    
    var imagePrompt: String {
        if lowercased().hasPrefix("draw") {
            return self.deletingPrefix("draw")
        } else if hasPrefix("vẽ") {
            return self.deletingPrefix("vẽ")
        }
        return self
    }
